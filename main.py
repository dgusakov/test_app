...
def validate_rec(rec):
    ...
    if 'link' in rec and 'label' not in rec:
        raise InvalidYamlFormat(f'Links without labels are not allowed. Error occurred in {rec}')
    ...
    for el in template:
        try:
            if el['depends'] not in ids:
                raise DependencyNotFound(f'Dependency form {el} is not presented in template')
        except KeyError:
            pass
...
def upload_template():
    ...
    if file and allowed_file_ext(file.filename):
        if 'data' in request.form:
            filename = str(loads(request.form['data']).get('tmpl_id', '')).lower() + '.yaml'
        else:
            filename = secure_filename(file.filename.rsplit('.', 1)[0].lower() + '.yaml')
        file.save(join(app.config['UPLOAD_FOLDER'], filename))
        resp = jsonify({'message': f'Template successfully uploaded. tmpl_id={filename.rsplit(".", 1)[0].lower()}'})
        resp.status_code = 201
        return resp
    else:
        resp = jsonify({'message': f'Allowed file types are {app.config["ALLOWED_EXTENSIONS"]}'})
        resp.status_code = 400
        return resp
...
def list_templates():
    files = [f for f in os.listdir(app.config['UPLOAD_FOLDER']) if isfile(join(app.config['UPLOAD_FOLDER'], f))]
    files_with_params = []
    for f in files:
...
def delete_template(tmpl_id):
    if tmpl_id != "":
        ...
        if os.path.isfile(path):
            os.remove(path)
            resp = jsonify({'message': f'Template with tmpl_id={tmpl_id} successfully deleted!'})
        else:
...
def install_template(tmpl_id):
    if tmpl_id != "":
        try:
            ...
            resp = jsonify({'message': f'Template with tmpl_id={tmpl_id} successfully installed!'})
